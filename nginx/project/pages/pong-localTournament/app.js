...
function setDisplay() {
  ...
  document.getElementById("createdTournamentText").classList.add("d-none");
}
...
function playOffTable() {
  ...
  scope.classList.add("bg-transparent", "textLight", "fs-3");
...
function setRoundsAndDisplay(pongSocket, nbrPlayers) {
  const startButton = document.getElementById("start");
  if (new_local_tournament) {
    document.getElementById("formNbrPlayers").classList.add("d-none");
    document.getElementById("createTournamentButton").classList.add("d-none");
    document.getElementById("formContainer").classList.add("d-none");
    document.getElementById("startText").classList.remove("d-none");
    document.getElementById("createdTournamentText").classList.remove("d-none");
    ...
    startButton.addEventListener("click", () => {
      ...
      pongSocket.send(
        JSON.stringify({
          message_type: "start_local_tournament",
          total_players: nbrPlayers,
          round: `${round}`,
        })
      );
    });
...
function initSocket(roomCode, nbrPlayers) {
  ...
  const pongSocket = new WebSocket(
    `${webSocketUrl}pong/${roomCode}/?userId=${getUserIdFromToken()}&jwt=${accessToken}&type=localTournament`
  );

  pongSocket.onmessage = handleMessage;

  pongSocket.onopen = (event) => {
    if (pongSocket.readyState === WebSocket.OPEN) {
      playOffTable();
      const message = {
        message_type: "localTournament",
        players: playerNames,
      };
  ...
  document.addEventListener("keyup", (e) => {
    ...
    if (!hash?.startsWith("#/pong-localTournament/")) return;
    if (unlockControls) {
      if (e.key === "w") {
        pongSocket.send(
          JSON.stringify({
  ...
  setRoundsAndDisplay(pongSocket, nbrPlayers);
}

function generateFormPlayers(nbrPlayers) {
  ...
  for (let i = 0; i < nbrPlayers; i++) {
    ...
    form.innerHTML = `<label for="input-${i}" class="form-label textLight fs-2 font-quicky mt-3 mb-1">Nickname ${
      i + 1
    }:</label> <input class="form-control" type="text" id="input-${i}" name="input-${i}" aria-required="true" aria-label="Nickname for player ${
      i + 1
    }" required>`;
...
function createPlayersList(nbrPlayers) {
  for (let i = 0; i < nbrPlayers; i++) {
    const playerName = document.getElementById(`input-${i}`).value;
    playerNames.push(playerName);
  }
}

function duplicatesPlayers(playerList) {
  return playerList.some((value, index) => playerList.indexOf(value) !== index);
}
...
function shufflePlayersNames(playersList) {
...
function init() {
  ...
  const nbrPlayersSelector = document.getElementById("formNbrPlayers");
  const localTournamentButton = document.getElementById(
    "createTournamentButton"
  );
  let nbrPlayers = parseInt(nbrPlayersSelector.value);
  generateFormPlayers(nbrPlayers);
  nbrPlayersSelector.addEventListener("change", function () {
    nbrPlayers = parseInt(nbrPlayersSelector.value);
    generateFormPlayers(nbrPlayers);
  });
  ...
  localTournamentButton.addEventListener("click", () => {
    ...
    createPlayersList(nbrPlayers);
    if (duplicatesPlayers(playerNames)) {
      document.getElementById("alert_duplicate").classList.remove("d-none");
      playerNames = [];
      return;
    }

    playerNames = shufflePlayersNames(playerNames);
    document.getElementById("alert_duplicate").classList.add("d-none");
